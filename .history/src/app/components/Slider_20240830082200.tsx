...
export const getPhotos = async () => { 
    const response = await fetch(baseUrl + <ENDPOINT></ENDPOINT>.PHOTO.GETPHOTOS, {
      method: 'GET',
    });
    if (!response.ok) {
...
export async function getServerSideProps() {
    try {
      ...
      const initialData = res.slice(1, 10); // Adjust slicing as needed
      ...
      console.error(error);
      return {
        props: { initialData: [] }, // Return an empty array or handle error state
      };
    }
  }
